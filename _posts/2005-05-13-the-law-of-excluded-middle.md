...
> _For every proposition $p$, either $p$ or $\lnot p$ holds._ 
...
> _For every proposition $p$, if $\lnot p$ does not hold, then $p$ holds._ 
...
> $\forall x \in D . P(x) \lor \lnot P(x)$. 
...
Imagine that the set $D$ in the above formulation of LEM is not just an ordinary set but a metric space, or more generally a topological space. Imagine furthermore that the property $P$ is restricted to be an open subset of $D$, that $\lnot P$ means the topological exterior of $P$, and that $\lor$ is union (which it is, actually). So now, is it necesssarily the case that $P \lor \lnot P$ covers all of $D$? It depends on $P$, does it not? For example, if we take $D = \mathbb{R}$, the real numbers with the usual metric, and $P = \{x \in \mathbb{R} ; x > 0\}$ then $\lnot P = \{x \in \mathbb{R} ; x < 0\}$ so $P \lor \lnot P = \mathbb{R} - \{0\}  \neq  \mathbb{R}$! But if we take $D = \mathbb{N}$, the natural numbers with discrete topology, then no matter what $P$ we choose $P$ and its exterior cover $\mathbb{N}$. 
