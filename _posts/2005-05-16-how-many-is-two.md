...
How many elements does $\Omega$ have? The constants $\top$ and $\bot$ which represent truth and falsehood, respectively, are truth values. So we have $\top \in \Omega$ and $\bot \in \Omega$. That's two elements. In classical logic, these are the only two, by which we mean that every $p \in \Omega$ is equal either to $\top$ or to $\bot$ (because either $p$ or $\lnot p$). In constructive logic, however, this is not the case. But this does not mean there is a third truth value, which is different from both $\top$ and $\bot$! How can that be? In order to explain this properly, we must be careful about what it means for a set to have “two elements”: For a set $A$, say that:

  * _$A$ has_ _two elements in the strong sense_ if there are $x, y \in A$ such that $x  \neq  y$ and, for all $z \in A$ either $z = x$ or $z = y$.
  * _$A$ has_ _two elements in the weak sense_ if there are $x, y \in A$ such that $x  \neq  y$ and, for all $z \in A$ it is not the case that $z$ is distinct from both $x$ and $y$.
...
To a classically trained mind there is no difference between the two senses of having two elements. Computationally speaking, however, there is a difference:
  ...
  * $A$ has two elements in the strong sense if we can compute $x \in A$ and $y \in A$ such that $x  \neq  y$, and there is an algorithm for deciding, given any $z \in A$, whether $z = x$ or $z = y$.
  * $A$ has two elements in the weak sense if we can compute $x \in A$ and $y \in A$ such that $x  \neq  y$, and there is no $z \in A$ that is distinct from both $x$ and $y$ (but there may be no algorithm for deciding, given $z \in A$, whether it equals $x$ or $y$).

Clearly, having two elements in the strong sense requires more (a decision procedure) than having two elements in the weak sense. In classical logic $\Omega$ has two elements in the strong sense. In constructive logic it has two elements in the weak sense, which we can prove as follows. Take $x = \top$ and $y = \bot$. Given any $z \in \Omega$ we must prove $\lnot (z  \neq  x \land z  \neq  y)$, which is logically equivalent to $\lnot (\lnot z \land \lnot \lnot z)$, which is true (exercise, someone post a solution please).
...
> $2 = \{p \in \Omega ; p \lor \lnot p\}$.

What are the elements of $2$? Since $\top \lor \lnot \top = \top \lor \bot = \top$ and $\bot \lor \lnot \bot = \bot \lor \top = \top$ we have $\top \in 2$ and $\bot \in 2$. And given any $p \in 2$, we know that $p \lor \lnot p$. If $p$ then $p = \top$, and if $\lnot p$ then $p = \bot$. We have proved that $2$ has two elements in the strong sense.

To add to the confusion, there is also the set of classical truth values,

> $Omega_c = \{p \in \Omega ; p \iff \lnot \lnot p\}$.

It consists of those truth values that satisfy the rule Reductio ad Absurdum (proof by contradiction).  
Since $\lnot \lnot \top = \lnot \bot = \top$ and $\lnot \lnot \bot = \lnot \top = \bot$ we see that $\top \in Omega_c$ and $\bot \in Omega_c$. We thus have a chain of inclusions

> $2 \subseteq Omega_c \subseteq \Omega$.
